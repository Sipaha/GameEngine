...
public class RenderBuffer {
    ...
    private static final RenderBuffer dynamicBuffer = new RenderBuffer(DEFAULT_SIZE);
    ...
    protected RenderBuffer(int capacity) {
        setSize(capacity);
    }

    public void render(Iterable<RenderUnit> renderUnits) {
        for(RenderUnit unit : renderUnits) unit.render(this);
    }
    ...
    /*public boolean setRenderDataTo(RenderUnit unit) {
        int newSize = size+unit.getRenderSize();
        if(newSize < buffer.length) {
            unit.setRenderData(buffer, size);
            size = newSize;
            return true;
        } else return false;
    }*/
    ...
    public int getAndAddSize(int amount) {
        int s = size;
        size += amount;
    ...
    public static RenderBuffer getStaticBuffer() {
        return staticBuffersPool.size > 0 ? staticBuffersPool.pop() : new RenderBuffer(DEFAULT_SIZE);
    }
}
